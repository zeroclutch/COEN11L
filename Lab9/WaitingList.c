...
void* autoSave(void* filename) {
        ...
        while(1) {
            printf("Starting autosave...\n");
            pthread_mutex_lock (&mutex);
            FILE* file = fopen(filename, "wb+");
             while (p->next != NULL) {
                printf("Writing %s to bin...\n", p->name);
                fwrite (&p->name, strlen(p->name) + 1, 1, filename);
                fwrite (&p->size, sizeof(p->size) + 1, 1, filename);
                p = p->next;
            };
            fclose(filename);
            pthread_mutex_unlock (&mutex);
            printf("Completed autosave");
            sleep(15);
        }
    return NULL;
}
...
int error;
void runAutoSave(void* filename) {
    error = pthread_create(&tid[1], NULL, &autoSave, filename);
    if(error) {
